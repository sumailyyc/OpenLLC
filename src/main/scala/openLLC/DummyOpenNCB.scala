...
class DummyOpenNCB(numRNs: Int = 1)(implicit p: Parameters) extends LazyModule with HasOpenLLCParameters {
  ...
  lazy val module = new DummyOpenNCBImp(numRNs)(this)
}

class DummyOpenNCBImp(numRNs: Int)(wrapper: DummyOpenNCB) extends LazyModuleImp(wrapper)
